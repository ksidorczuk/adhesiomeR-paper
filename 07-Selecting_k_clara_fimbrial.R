...
dist_fimbrial <- dist(x = as.matrix(fimbrial_x), method = "manhattan")


## Clustering - gene presence/absence
kmax = 200

clara_gap <- fviz_nbclust(fimbrial_x, clara, method = "gap_stat", k.max = kmax, diss = dist_fimbrial, nboot = 10)
ggsave(paste0(data_path, "Clustering/SelectingK/clara_gapstat_fimbrial_man.png"), (clara_gap + theme(axis.text.x = element_text(angle = 90))), height = 6, width = 35)

save(list = ls(), file = paste0(data_path, "Clustering/SelectingK/clustering_data_clara_fim_man.RData"))
